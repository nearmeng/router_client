...
BOOL CRouterClient::router_msg_handle(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	if (tbus_get_type(nSrcAddr) == ms_RouterClient.m_nConnServerType)
	{
...
BOOL CRouterClient::init(int32_t nTbusHandle, int32_t nTbusID, int32_t nConnServerType, 
	int32_t nRouterServerType, MESSAGE_FUNC pMsgHandle, BOOL bResume)
{
	int32_t nRetCode = 0;
	...
	m_nTbusID = nTbusID;
	m_nConnServerType = nConnServerType;
	m_nRouterServerType = nRouterServerType;
	m_nRouterTbusID = 0;
	...
	for (int32_t nIndex = 0; nIndex < nPeerCount; nIndex++)
	{
		...
		if (nServerType == m_nRouterServerType)
		{
